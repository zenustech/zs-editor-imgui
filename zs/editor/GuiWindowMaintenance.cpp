  ...
  void GUIWindow::InteractiveStates::updateMonitors() {
    ...
    for (int n = 0; n < monitorCount; n++) {
      ...
      monitor.PlatformHandle
          = (void *)glfwMonitors[n];  // [...] GLFW doc states: "guaranteed to be valid
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n++) {
      ...
      if (is_window_focused) {
        ...
        if (states.mouseWindow == nullptr) {
          ...
          states.lastValidMousePos = zs::vec<float, 2>((float)mouse_x, (float)mouse_y);
...
#  endif
  ...
  void GUIWindow::updateImguiMouseCursor() {
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n++) {
      ...
      if (imgui_cursor == ImGuiMouseCursor_None || io.MouseDrawCursor) {
        ...
        glfwSetCursor(window, get_imgui_mouse_cursor(imgui_cursor)
  ...
  bool GUIWindow::beginFrame() {
    ...
    if (states.wantUpdateMonitors) states.updateMonitors();
    ...
    if (current_time <= states.time) current_time = states.time + 0.00001f;
    io.DeltaTime = states.time > 0.0 ? (float)(current_time - states.time) : (float)(1.0f / 60.0f);
    ...
    states._eventQueue.postEvents(&refGlobalWidget(), &unhandled);
    tryConsumingRemainingEvents(unhandled);
    /// @note more complex events might be newly setup
    states._eventQueue.postEvents(&refGlobalWidget(), nullptr);
  ...
  void GUIWindow::endFrame() {
    ...
    if (!resized) swapchain.nextFrame();
  }
