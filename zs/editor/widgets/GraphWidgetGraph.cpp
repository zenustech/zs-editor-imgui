...
void Graph::paint() {
...
#endif
        {
          ...
          if (/*self link*/ srcPin->_node == dstPin->_node ||
            ...
            if (ed::AcceptNewItem()) {
              ...
              if (dstPin->_links.size())
                _links.erase((*dstPin->_links.begin())->_id);
              ///
              auto lid = nextLinkId();
              auto [iter, success] = _links.emplace(
                  std::piecewise_construct, std::forward_as_tuple(lid),
  ...
  if (ImGui::BeginPopup((const char *)u8"创建节点")) {
    Node *node = nullptr;
    if (ImGui::MenuItem((const char *)u8"节点"))
      node = spawnDummyNode();
  ...
  if (ed::GetSelectedObjectCount() == 0)
    ImGui::SetKeyOwner(ImGuiMod_Alt, _prevAltOwner);
}
...
///
zs::tuple<Graph::NodeMap::iterator, bool>
Graph::spawnNode(std::string_view name, ed::NodeId nid) {
  updateObjectId(nid.Get());
  auto [iter, success] =
      _nodes.emplace(std::piecewise_construct, std::forward_as_tuple(nid),
                     std::forward_as_tuple(nid, this, name));
  if (!success)
    throw std::runtime_error(
        "unable to create a new node (maybe due to duplication).");
  return zs::make_tuple(iter, success);
}
zs::tuple<Graph::NodeMap::iterator, bool>
Graph::spawnNode(std::string_view name) {
  return spawnNode(name, nextNodeId());
}
zs::tuple<Graph::LinkMap::iterator, bool> Graph::spawnLink(Pin *srcPin,
                                                           Pin *dstPin) {
  auto lid = nextLinkId();
  auto [iter, success] =
  ...
  if (!success)
    throw std::runtime_error(
        "unable to create a new link (maybe due to duplication).");
  return zs::make_tuple(iter, success);
}
