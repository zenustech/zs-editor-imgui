...
static int InputTextCallback(ImGuiInputTextCallbackData *data) {
  Zs_InputTextCallback_UserData *user_data = (Zs_InputTextCallback_UserData *)data->UserData;
...
namespace zs {
    ...
    void Pin::paint(float width) {
      ...
      if (_kind == pin_kind_e::Input) {
        ...
        ax::Widgets::Icon(ImVec2(static_cast<float>(get_pin_icon_size()),
                          ...
                          getIconType(), connected, getIconColor(), ImColor(32, 32, 32, alpha));
        ed::EndPin();
        ...
        appendixZoneStartX = ImGui::GetItemRectMin().x;  // retrieved after first item

        /// 1-label
        offsetX += ImGui::CalcTextSize(_name.c_str()).x + ImGui::GetStyle().ItemSpacing.x;
        ...
        if (expandable()) {
          ...
          if (isExpanded) {
            ...
            ImGui::TextColored(editorStyle.Colors[ed::StyleColor_PinRectBorder], offTag.data());
            if (ImGui::IsItemClicked(ImGuiMouseButton_Left)) _expanded = false;

            ImVec2 labelSize = ImGui::CalcTextSize(_name.c_str());
            drawList->AddRect(
                pos - ImVec2(editorStyle.PinRounding, editorStyle.PinRounding) * 0.5,
                pos + labelSize + ImVec2(editorStyle.PinRounding, editorStyle.PinRounding) * 0.5,
            ...
            ImGui::TextColored(editorStyle.Colors[ed::StyleColor_PinRectBorder], onTag.data());
            if (ImGui::IsItemClicked(ImGuiMouseButton_Left)) _expanded = true;
          }
        }
        ...
        if (canEdit) {
          ...
          ImGui::InputText(
              textLabel.c_str(), &contentStr,
              ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_AutoSelectAll);
#elif 0
          std::string &contentStr = (*_content);
          ImGui::InputTextEx(
              textLabel.c_str(), NULL, content, (*_content).capacity(), sz,
              ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_AutoSelectAll);
#else
          std::string &contentStr = (*_content);
          ...
          ImGui::InputTextEx(textLabel.c_str(), NULL, content, (*_content).capacity() + 1, sz,
        ...
        if (appendixZoneEndX > _node->_nodeBound) _node->_nodeBound = appendixZoneEndX;

        ImGui::SetCursorPosY(pinRectMax.y + ImGui::GetStyle().ItemSpacing.y);

      } else {
        float offset = 0.f;
        auto boundX = width - ImGui::CalcTextSize(_name.c_str()).x - get_pin_icon_size()
                      - ImGui::CalcTextSize("+").x - spacing.x / 2
                      - ImGui::GetStyle().ItemSpacing.x * 4;

        enum stage_e : u32 { stage_content = 0, stage_switch, stage_label, stage_icon, num_stages };
        ...
        // 0-content
        if (canEdit) {
          dims[stage_content] = ImVec2(spacing.x, 0) +
          ...
          dims[stage_content].x = std::max(get_min_content_size(), dims[stage_content].x);
        ...
        if (expandable()) {
          ...
          if (headStage > stage_switch) headStage = stage_switch;
        ...
        if (dims[stage_content].x != 0.f) {  // output pin content is editable at all time
        ...
        if (pinRectMax.x > _node->_nodeBound) _node->_nodeBound = pinRectMax.x;

        if (!isExpanded) appendixZoneEndX = ImGui::GetItemRectMax().x;  // retrieved at the end
      ...
      ImRect appendixTriggerZone{
          ImVec2{appendixTriggerZoneStartX, appendixZoneStartY + ImGui::GetStyle().ItemSpacing.y
                                                - get_half_insertion_zone_span()},
          ImVec2{appendixTriggerZoneEndX, appendixZoneStartY + ImGui::GetStyle().ItemSpacing.y
                                              + get_half_insertion_zone_span()}};
      ...
      if (appendixTriggerZone.Contains(ImGui::GetMousePos())) {
        ...
        if (itemCreator.IsDragging() && itemCreator.m_DraggedPin
          ...
          if (ImGui::IsMouseReleased(ed::GetConfig().DragButtonIndex)) {
            ...
            _node->_graph->enqueueMaintenanceAction([this, graph = _node->_graph]() {
              ...
              if (expanded()) {  // child-level appendix
                ...
                auto chPin = prepend(id, std::string("item_") + std::to_string(id.Get()));
                auto srcPin = graph->_pins.at(itemCreator.m_DraggedPin->m_ID);
                auto dstPin = chPin;
                if (srcPin->_kind == pin_kind_e::Input && dstPin->_kind == pin_kind_e::Output)
                ...
                if (_parent) {
                  ...
                  auto id = graph->nextPinId();
                  auto chPin
                      = _parent->append(this, id, std::string("item_") + std::to_string(id.Get()));
                  auto srcPin = graph->_pins.at(itemCreator.m_DraggedPin->m_ID);
                  auto dstPin = chPin;
                  if (srcPin->_kind == pin_kind_e::Input && dstPin->_kind == pin_kind_e::Output)
                    zs_swap(srcPin, dstPin);
                  ...
                  Pin *chPin = nullptr;
                  if (_kind == pin_kind_e::Input)
                    chPin = _node->appendInput(this, id,
                                               std::string("item_") + std::to_string(id.Get()));
                  else if (_kind == pin_kind_e::Output)
                    chPin = _node->appendOutput(this, id,
                                                std::string("item_") + std::to_string(id.Get()));
                  auto srcPin = graph->_pins.at(itemCreator.m_DraggedPin->m_ID);
                  auto dstPin = chPin;
                  if (srcPin->_kind == pin_kind_e::Input && dstPin->_kind == pin_kind_e::Output)
            ...
            drawList->AddRectFilled(
                ...
                ImVec2(appendixZoneEndX, appendixZoneStartY + ImGui::GetStyle().ItemSpacing.y * 2),
                ImGui::ColorConvertFloat4ToU32(editorStyle.Colors[ed::StyleColor_Flow]),
  ...
  if (hasContents()) {
    ...
    Pin *Pin::prepend(const ed::PinId &id, std::string_view label) {
      auto ret = _chs.emplace(std::begin(_chs), id, _node, label, _type, _kind, this);
    ...
    Pin *Pin::append(const Pin *target, const ed::PinId &id, std::string_view label) {
      auto it = std::begin(_chs);
      for (; it != std::end(_chs); ++it)
        if (zs::addressof(*it) == target) break;
      if (it != std::end(_chs)) it++;
      ...
      if (ImGui::IsItemClicked()) {
        if (ImGui::GetMouseClickedCount(ImGuiMouseButton_Left) == 2) {
          ...
          _node->_graph->setupAuxiliaryWidgetAction(
              popupId, [this, popupTag, popupId, newName = std::string(_name)]() mutable {
                ed::Suspend();
                if (ImGui::BeginPopup(popupTag.c_str())) {
                  if (ImGui::InputText("##edit", &newName,
                    ...
                    _node->_graph->enqueueMaintenanceAction([graph = _node->_graph, popupId] {
    ...
    bool Pin::isPinLinked() const {
      if (_links.size() > 0) return true;
      if (expanded()) return false;
      for (auto &ch : _chs)
        if (ch.isPinLinked()) return true;
      return false;
    }

    float Pin::evaluatePinWidth(float offset) const {
      auto inherentSz = offset + get_pin_icon_size()
      ...
      if (editable()) {
        ...
        auto contentSz = ImGui::GetStyle().ItemInnerSpacing.x + _contentWidget.preferredWidth();
        contentSz += ImGui::CalcTextSize(" ").x;
        if (contentSz > contentWidth) contentWidth = contentSz;

        sz += contentWidth + ImGui::GetStyle().ItemSpacing.x + ImGui::GetStyle().ItemInnerSpacing.x;
      }
      if (expandable()) {
      ...
      if (contents()) {  // check validity of the inserted ZsVar
        ...
        if (ts.size() > 5 && ts.substr(0, 4) == "enum") {
          if (contents) {
            ...
            if (obj) setupContentWidget(l.asList());
          } else {
    ...
    zs::ui::GenericResourceWidget &Pin::setupContentWidget(ZsValue aux) {
      if (!aux) _contentWidget = zs::ui::GenericResourceWidget(_name, contents(), aux);
#if 0
  if (aux) {
    ...
    _contentWidget.injectCustomBehavior(
      ...
      if (aux) {
        _contentWidget
            = zs::ui::make_generic_resource_widget<zs::ui::ComboSlider>(_name, contents(), aux);
      }
#endif
