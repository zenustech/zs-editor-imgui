...
namespace zs {
  ...
  namespace ge {  // graph editor
    ...
    struct Pin {
      ...
      static float get_half_insertion_zone_span() noexcept { return get_pin_icon_size() * 0.4f; }
      ...
      Pin(Pin &&o) noexcept
          : _id{zs::exchange(o._id, ed::PinId{0})},
            _node{zs::exchange(o._node, nullptr)},
            _name{zs::move(o._name)},
            _expanded{zs::move(o._expanded)},
            _type{o._type},
            _kind{o._kind},
            _links{zs::move(o._links)},
            _chs{zs::move(o._chs)},
      ...
      [[maybe_unused]] Pin *append(const Pin *target, const ed::PinId &id, std::string_view label);
      void remove_back() {
        if (!_chs.empty()) _chs.pop_back();
      }
      ...
      bool expanded() const noexcept {
        if (_expanded) return _expanded.value();
        return false;
      }
      bool expandable() const noexcept { return _expanded.has_value(); }
      bool hasContents() const noexcept { return _contentWidget; }
      bool editable(bool connected = false) const noexcept { return !connected && hasContents(); }
    ...
    struct Node {
      ...
      Node(Node &&o) noexcept
          : _id{zs::exchange(o._id, ed::NodeId{0})},
            _name{zs::move(o._name)},
            _graph{zs::exchange(o._graph, nullptr)},
            _subgraph{zs::move(o._subgraph)},
            _inputs{zs::move(o._inputs)},
            _outputs{zs::move(o._outputs)},
            _pos{o._pos},
            _type{o._type},
            _nodeWidth{o._nodeWidth},
            _nodeBound{o._nodeBound},
            cmdText{zs::move(o.cmdText)},
            helperText{zs::move(o.helperText)},
      ...
      Node(ed::NodeId id, Graph *graph, std::string_view name, ImVec2 pos = ImVec2(0, 0))
          : _id{id},
            _graph{graph},
            _subgraph{},
            _name{name},
            _pos{pos},
            _type{node_type_e::Blueprint},
            cmdText((std::size_t)128, '\0'),
            _attribs{},
            _attribWidget{} {}
      ~Node();
      ...
      [[maybe_unused]] Pin *appendInput(const ed::PinId &id, std::string_view label);
      [[maybe_unused]] Pin *appendOutput(const ed::PinId &id, std::string_view label);
      ...
      Pin *findInputPin(std::string_view tag) {
        for (auto &input : _inputs)
          if (input._name == tag) return &input;
        return nullptr;
      }
      Pin *findOutputPin(std::string_view tag) {
        for (auto &output : _outputs)
          if (output._name == tag) return &output;
        return nullptr;
      }
    ...
    struct Link {
      ...
      Link(ed::LinkId id, Pin *src, Pin *dst)
          : _id{id}, _startPinID{src->_id}, _endPinID{dst->_id}, _srcPin{src}, _dstPin{dst} {
    ...
    struct Graph {
      ...
      Graph(Graph &&o) noexcept
          : _nextObjectId{zs::exchange(o._nextObjectId, (u32)0)},
            ...
            _edCtx{zs::exchange(o._edCtx, nullptr)},
            _links{zs::move(o._links)},
            _pins{zs::move(o._pins)},
            _nodes{zs::move(o._nodes)},
            _viewJson{zs::move(o._viewJson)},
            _viewRect{zs::move(o._viewRect)},
      ...
      Graph(std::string_view name, std::string_view filename)
          : _nextObjectId{1}, _name{name}, _fileName{filename}, _initRequired{false}, _viewJson{} {
        hoveredNode = 0;
        hoveredPin = 0;
        hoveredLink = 0;
        numSelectedNodes = 0;
      ...
      ed::NodeId lastNodeId() const {
        if (!_nodes.empty()) return (*std::prev(std::end(_nodes))).first;
      ...
      void clearAuxiliaryWidgetAction(ImGuiID id) { _auxiliaryWidgetActions.erase(id); }

      zs::tuple<NodeMap::iterator, bool> spawnNode(std::string_view name, ed::NodeId nid);
      ...
      Node *findNode(ed::NodeId id) {
        if (auto it = _nodes.find(id); it != _nodes.end()) return zs::addressof(it->second);
        return nullptr;
      }
      Node *findNode(std::string_view tag) {
        for (auto &[id, node] : _nodes)
          if (node._name == tag) return &node;
        return nullptr;
      }
      Pin *findPin(ed::PinId id) {
        if (auto it = _pins.find(id); it != _pins.end()) return it->second;
        return nullptr;
      }
    ...
    protected:
      Graph() noexcept : _nextObjectId{1}, _edCtx{nullptr}, _initRequired{false}, _viewJson{} {}
