...
void Node::paint() {
  ...
  auto HeaderMin =
      ImGui::GetItemRectMin() + ImVec2(0, -ImGui::GetStyle().ItemSpacing.y * 2);
  ...
  _nodeWidth = std::max(lw + rw, headerWidth - HeaderMin.x + 16 -
  ...
  ImGui::SetCursorPosY(
      ...
      ImGui::GetStyle().ItemInnerSpacing.y + editorStyle.NodeBorderWidth * 2);
  ...
  if (ImGui::IsItemVisible()) {
    ...
    const auto uv = ImVec2((HeaderMax.x - HeaderMin.x) /
                               (float)(4.0f * /*HeaderTextureWidth*/ 32),
                           (HeaderMax.y - HeaderMin.y) /
                               (float)(4.0f * /*HeaderTextureHeight*/ 32));
    auto headerColor = IM_COL32(0, 0, 0, 255) |
                       (ImColor(255, 255, 255) & IM_COL32(255, 255, 255, 0));
...
Pin *Node::appendInput(const Pin *target, const ed::PinId &id,
                       std::string_view label) {
  auto it = std::begin(_inputs);
  for (; it != std::end(_inputs); ++it)
    if (zs::addressof(*it) == target)
      break;
  if (it != std::end(_inputs))
    it++;
  auto ret =
      _inputs.emplace(it, id, this, label, pin_type_e::Flow, pin_kind_e::Input);
  return _graph->_pins[id] = zs::addressof(*ret);
}
Pin *Node::appendOutput(const Pin *target, const ed::PinId &id,
                        std::string_view label) {
  ...
  if (it != std::end(_outputs))
    it++;
  auto ret = _outputs.emplace(it, id, this, label, pin_type_e::Flow,
                              pin_kind_e::Output);
  return _graph->_pins[id] = zs::addressof(*ret);
}
...
float Node::evaluateInputPinWidth() const {
  float width = 0;
  for (auto &pin : _inputs) {
    if (auto w = pin.evaluatePinWidth(); w > width)
      width = w;
  }
  return width;
}
  ...
  if (res.second && res.first->second) { // check validity of the inserted ZsVar
    ...
    if (ts.size() > 5 && ts.substr(0, 4) == "enum") {
      if (contents) {
        ...
        if (obj)
          _attribWidget.appendWidget(label, obj, l.asList());
      } else {
    ...
    /// int
      ...
      if (obj)
        _attribWidget.appendWidget(label, obj);
    }
    /// float
    else if (ts == "float") {
