...
Node *Graph::spawnDummyNode() {
  ...
  auto [iter, success] = _nodes.emplace(
      ...
      std::forward_as_tuple(id, this,
                            std::string("node") + std::to_string(id.Get())));
  if (success) {
...
static void savePinAttribs(Json &entryJson, const Pin &pin) {
  if (pin.expandable())
    entryJson["expansion"] = (int)pin.expanded();
  if (pin.hasContents()) {
    {
      GILGuard guard;
...
void Graph::save() {
  ...
  }
  {
    ...
    viewJson["scroll"] =
        std::array<float, 2>{canvasView.Origin.x, canvasView.Origin.y};
...
static Pin *locatePin(const std::string &p, Graph &graph,
  ...
  PinList *candidates =
      pinKind == pin_kind_e::Input ? &node->_inputs : &node->_outputs;
  bool found = false;
  do {
    found = false;
...
static void loadPinAttribs(const Json &pinAttribs, Pin *pin) {
  ...
  if (auto it = pinAttribs.find("expansion"); it != pinAttribs.end()) {
    if ((*it).is_number_integer())
      pin->_expanded = static_cast<int>(*it);
  }
...
void Graph::init(std::string_view filename) {
...

  {
    ...
    for (auto &linkJson : linksJson) {
      ...
      auto srcPinPtr = locatePin(linkJson["src_pin_path"].get<std::string>(),
                                 *this, pin_kind_e::Output);
      auto dstPinPtr = locatePin(linkJson["dst_pin_path"].get<std::string>(),
                                 *this, pin_kind_e::Input);
#endif
      if (srcPinPtr && dstPinPtr)
        auto [iter, success] = spawnLink(srcPinPtr, dstPinPtr);
      // ed::Link(iter->first, iter->second._startPinID,
      // iter->second._endPinID);
    }
    /// view
    Json &viewJson = json["view"];
    ImVec2 viewOrigin{(float)viewJson["scroll"][0],
                      (float)viewJson["scroll"][1]};
...
void Graph::acquireEditorContext(std::string_view name) {
  ...
  if (ImguiSystem::has_node_editor(_name)) {
    ...
    const_cast<ed::Detail::Config &>(getEditorContext()->GetConfig())
        .UserPointer = this;
  } else {
    ed::Config config;
...
const std::vector<Node *> &Graph::querySelectionNodes() {
  ...
  for (int i = 0; i != n; ++i)
    selectedNodes[i] = &_nodes.at(res[i]);
