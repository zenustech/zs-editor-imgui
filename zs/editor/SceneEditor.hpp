...
glm::mat4 get_transform(const vec<float, 3> &translation,
                        const vec<float, 3> &eulerXYZ,
...
struct SceneEditor {
  ...
  struct SceneEditorInputMode {
    void update(float dt) {
      match([](std::monostate) {},
            [dt](auto &input) { input.get().update(dt); })(_modes);
    }
    void paint() {
      match([](std::monostate) {},
            [](auto &input) { input.get().paint(); })(_modes);
    }
    ...
    variant<std::monostate, Owner<SceneEditorRoamingMode>,
  ...
  glm::vec3 getCameraPosition() const noexcept {
    return -sceneRenderData.camera.get().position;
  }
  ...
  bool showWireframe{true}, showIndex{true}, showCoordinate{true},
      enableGuizmo{true}, ignoreDepthTest{false}, showNormal{true},
      showOutline{true}, drawTexture{false};
  enum draw_pipeline_e : u32 {
    draw_Color = 0,
    draw_Texture,
    num_DrawPipelines
  };
  ...
  struct {
    Signal<void(std::string_view fileLabel, std::string_view nodePath)>
        _transform;
  } _signals;
  ...
  VulkanContext &ctx() {
    return const_cast<VulkanContext &>(guiRenderer->ctx());
  }
  ...
  double scenePassTime, pickPassTime, gridPassTime, textPassTime,
      textRenderPassTime;
  ...
  void resetDrawStates() {
    for (auto &[_, drawn] : currentVisiblePrimsDrawn)
      drawn = 0;
    sceneRenderData.sceneCtx = &getCurrentScene();
    sceneRenderData.currentTimeCode =
        sceneRenderData.sceneCtx->getCurrentTimeCode();
  }
  void resetVkCmdCounter() {
    for (auto &num : currentlyUsedSecondaryCmdNum)
      num = 0;
  }
  ...
  inline auto evalRenderChunkSize(int nWork) const noexcept {
    return std::max(1, (int)((nWork + renderScheduler->numWorkers() - 1) /
                             renderScheduler->numWorkers()));
  }
  ...
  using VisBufferStates =
      std::variant<DisplayingVisBuffers, PreProcessingVisBuffers,
                   PostProcessingVisBuffers>;
  ...
  struct VisBufferStateBase {
    ...
    template <typename Event> OptionalState process(const Event &event) {
      return {};
    }
  ...
  struct VisBufferHandler {
    ...
    template <typename Event> void process(const Event &event) {
      auto ret = std::visit(
          [&event](auto &state) -> OptionalState {
  ...
  struct SceneRenderData {
    ...
    std::string sceneLabel{
        g_defaultSceneLabel}; // through this the scene is drawn
  ...
  const auto &getCurrentVisiblePrims() const noexcept {
    return currentVisiblePrims;
  }
  ...
  auto getCurrentScenePrimsRecurse() {
    ...
    zs::function<void(Weak<ZsPrimitive>)> gatherChildren =
        [&](Weak<ZsPrimitive> prim_) {
  ...
  ZsPrimitive *getScenePrimByIndex(int i) {
    return getCurrentScene().getPrimByIndex(i);
  }
